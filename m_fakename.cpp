/* $ModDesc: Provides inter-server FAKENAME */
...
/** Handle /FakeName
 */
class CommandFakeName : public Command, public NPCx
{
public:
	CommandFakeName(Module *parent) : Command(parent, "FAKENAME", 3, 3), NPCx("FAKENAME")
	{
...
class ModuleFakeNameCommand : public Module
{
	CommandFakeName fake_name;

public:
	ModuleFakeNameCommand() : fake_name(this)
	{
    }

	~ModuleFakeNameCommand()
	{
	...
	void init()
	{
		ServiceProvider *services[] = { &this->fake_name, };
	...
	Version GetVersion()
	{
		return Version("Provides FakeName", VF_COMMON);
	}
	...
	ModResult OnPreCommand(std::string &command, std::vector<std::string> &parameters, LocalUser *user, bool validated, const std::string &original_line)
	{
		...
		if (command == "FAKENAME")
		{
			...
			text = sep.GetRemaining();
				this->fake_name.SetText(text);
		}
		return MOD_RES_PASSTHRU;
	}
};

MODULE_INIT(ModuleFakeNameCommand)
